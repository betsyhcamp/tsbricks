...
- **WorkdayNormalizeTransform**: Normalizes a target column by the number of working days per period using a user-provided calendar DataFrame. Supports both global calendars (all series share the same working-day counts) and per-series calendars (working-day counts vary by series, e.g., different regions with different holidays). Requires an explicit `calendar_scope` parameter (`"global"` or `"per_series"`) and a `calendar_df` parameter (a pandas DataFrame covering both historical and forecast-horizon periods). The calendar DataFrame uses a fixed column name `n_workdays`. Config `scope` must be `global` (validated at the Pydantic layer). See `spec_workday_normalize_transform.md` for the full feature specification.
