...
class BoxCoxTransform(BaseTransform):
    ...
    def transform(self, df: pd.DataFrame, target_col: str) -> pd.DataFrame:
        ...
        def _apply(series_df: pd.DataFrame, col: str, uid: str) -> pd.DataFrame:
    ...
    def inverse_transform(self, df: pd.DataFrame, target_col: str) -> pd.DataFrame:
        ...
        def _invert(series_df: pd.DataFrame, col: str, uid: str) -> pd.DataFrame:
            lmbda = self._fitted_lambdas[uid]
            values = series_df[col].to_numpy(dtype=float)
            series_df = series_df.copy()
    ...
    def get_fitted_params(self) -> dict[str, dict]:
        return {
            uid: {"lambda": float(lmbda)} for uid, lmbda in self._fitted_lambdas.items()
        }
